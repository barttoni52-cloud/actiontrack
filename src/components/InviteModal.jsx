...
export default function InviteModal({ onClose, users, currentUser }) {
  ...
  const invite = async () => {
    ...
    try {
      ...
      const { error: signErr } = await supabaseSignup.auth.signUp({
        ...
        options: {
          data: {
            nom: form.nom, role: form.role, poste: form.poste,
            service: form.service, avatar,
  ...
  return (
    <Modal onClose={onClose} maxWidth={480}>
      ...
      <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:8, padding:'10px 14px', fontSize:11, color:'#1d4ed8', marginBottom:16 }}>
        🔐 Un mot de passe sécurisé sera généré. Le nouvel utilisateur sera dans votre organisation.
      </div>
