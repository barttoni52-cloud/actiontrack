...
export default function ActionDetail({ actionId, actions, users, currentUser, onClose, onUpdate, onAddJournal, onQRScan }) {
  ...
  const addComment = () => {
    ...
    const updated = [
      ...action.commentaires,
      { id: gid('C'), auteurId: currentUser.id, texte: newComment.trim(), date: nowISO(), type: 'normal', photos: [...pendingPhotos] },
    ];
    onUpdate(actionId, { commentaires: updated });
    setNewComment('');
    setPendingPhotos([]);
  };
  ...
  const toggleEtape = (i) => {
    const updated = action.etapes.map((e, idx) =>
      idx === i ? { ...e, fait: !e.fait, dateFait: !e.fait ? nowISO() : null } : e
    );
  ...
  const applyStatut = () => {
    if (!newStatut || newStatut === action.statut) return;
    const patch = { statut: newStatut };
    if (newStatut === 'VALIDÉ') patch.dateFin = nowISO();
    onUpdate(actionId, patch);
    onAddJournal(actionId, `Statut → ${newStatut}`, newStatut === 'VALIDÉ' ? 'validation' : newStatut === 'REJETÉ' ? 'rejet' : 'mise_a_jour');
    setNewStatut('');
  };
      ...
      <Modal onClose={onClose} maxWidth={660}>
        ...
        {tab === 'detail' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {action.description && (
              <div style={{ background: '#f5f4f0', borderRadius: 8, padding: 12, color: '#4a4844', fontSize: 12, lineHeight: 1.6 }}>
                {action.description}
              </div>
            )}
            {action.retardMotif && (
            ...
            {canEdit && currentUser.role !== 'agent' && (
              <div style={{ background: '#f5f4f0', borderRadius: 8, padding: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <Select value={newStatut} onChange={e => setNewStatut(e.target.value)} style={{ flex: 1 }}>
                    <option value="">— Statut —</option>
                    {STATUTS.map(s => <option key={s}>{s}</option>)}
                  </Select>
                  <Btn variant="primary" onClick={applyStatut}>Appliquer</Btn>
                </div>
              </div>
            )}
          </div>
              ...
              {action.etapes.map((e, i) => (
                ...
                >
                  <div style={{
                    ...
                    border: `2px solid ${e.fait ? '#16a34a' : '#c4bfb8'}`,
                    background: e.fait ? '#16a34a' : 'transparent',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                  }}>
                    {e.fait && <span style={{ color: '#fff', fontSize: 10, fontWeight: 900 }}>✓</span>}
                  </div>
                  <span style={{ fontSize: 11, color: e.fait ? '#16a34a' : '#4a4844', flex: 1, textDecoration: e.fait ? 'line-through' : 'none' }}>
                    {e.titre}
                  </span>
        ...
        {tab === 'comm' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {action.commentaires.length === 0 && (
              <div style={{ textAlign: 'center', color: '#a09c98', padding: 24, fontSize: 12 }}>Aucun commentaire</div>
            )}
                ...
                <div key={j.id} style={{ display: 'flex', gap: 10, padding: '10px 0', borderBottom: i < action.journal.length - 1 ? '1px solid #f0ede8' : 'none' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 3, flexWrap: 'wrap' }}>
                      <span style={{ color: '#1a1a18', fontSize: 11, fontWeight: 700 }}>{auteur?.nom || '?'}</span>
                      <span style={{ fontSize: 9, color: JTYPE_COLORS[j.type] || '#a09c98', background: '#f0ede8', padding: '1px 6px', borderRadius: 99, fontWeight: 600 }}>
                        {j.type.replace('_', ' ')}
                      </span>
