...
export default function ValidatePage({ token }) {
  ...
  const loadAction = async () => {
    setLoading(true);
    const { data } = await supabase.from('actions').select('*').eq('qr_token', token).single();
    if (data) {
      setAction(data);
  ...
  const submit = async () => {
    ...
    const now = nowISO();
    const newComment = (comment || photos.length > 0) ? {
      id: gid('C'), auteurId: action.assigne_a, texte: comment,
  ...
  if (step === 'error') return (
    <div style={S.wrap}><div style={S.card}>
      <div style={{ textAlign: 'center', padding: '20px 0' }}>
        ...
        <div style={{ fontSize: 12, color: '#7a7672' }}>Ce QR code ne correspond à aucune action.</div>
      </div>
    </div></div>
  );
  ...
  return (
    <div style={S.wrap}>
      <div style={S.card}>
        ...
        <label style={S.label}>Commentaire sur la mission</label>
        <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Décrivez ce qui a été fait, les résultats obtenus..." style={{ ...S.input, height: 90, resize: 'vertical' }} />

        <div style={{ marginBottom: 14 }}>
          <div onClick={() => fileRef.current?.click()} style={{ border: '2px dashed #c4bfb8', borderRadius: 8, padding: '12px', textAlign: 'center', cursor: 'pointer', background: '#f5f4f0' }}>
            <div style={{ fontSize: 20, marginBottom: 4 }}>📷</div>
            <div style={{ fontSize: 11, color: '#7a7672' }}>Ajouter des photos ({photos.length}/3)</div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
              {photos.map((src, i) => (
                <div key={i} style={{ position: 'relative' }}>
                  <img src={src} alt="" style={{ width: 72, height: 72, objectFit: 'cover', borderRadius: 8, border: '1px solid #d4cfc8' }} />
                  <button onClick={() => setPhotos(p => p.filter((_, j) => j !== i))} style={{ position: 'absolute', top: -6, right: -6, background: '#dc2626', border: 'none', borderRadius: '50%', width: 18, height: 18, color: '#fff', fontSize: 10, cursor: 'pointer' }}>✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        {isOverdue && mode !== 'echec' && (
          <div style={{ marginBottom: 14 }}>
            <label style={S.label}>Motif du retard *</label>
            ...
            <textarea value={retardDetails} onChange={e => setRetardDetails(e.target.value)} placeholder="Expliquez..." style={{ ...S.input, height: 60, resize: 'vertical' }} />
          </div>
        )}

        {mode === 'echec' && (
          <div style={{ background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 10, padding: 14, marginBottom: 14 }}>
            <label style={{ ...S.label, color: '#dc2626' }}>Motif de non-réalisation *</label>
        ...
        {mode !== 'echec' ? (
          <>
            <button style={S.btnGreen} onClick={() => { setMode('valider'); submit(); }} disabled={submitting || (isOverdue && !retardMotif)}>
              {submitting ? '...' : '✓ Valider la mission'}
            </button>
