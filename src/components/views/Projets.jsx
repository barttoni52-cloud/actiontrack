...
export default function Projets({ projets, actions, users, currentUser, onSelectAction, setProjets }) {
  ...
  const addAgent = (userId) => {
    if (!userId || agentsProjet.includes(userId)) return;
    setAgentsProjet(p => [...p, userId]);
  };

  const removeAgent = (userId) => setAgentsProjet(p => p.filter(id => id !== userId));
  ...
  const create = () => {
    ...
    setProjets(p => [...p, {
      ...
      couleur: form.couleur, actif: true,
      agents: agentsProjet,
    }]);
        ...
        <Card>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 9, color: '#7a7672', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 6 }}>
              Agents assignés au projet ({agentsProjet.length})
            </div>
          ...
          <Field label="Couleur">
            <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
              {COLORS.map(c => (
                <div key={c} onClick={() => f('couleur', c)} style={{ width: 24, height: 24, borderRadius: '50%', background: c, cursor: 'pointer', border: `3px solid ${form.couleur === c ? '#1a1a18' : 'transparent'}`, transition: 'border .1s' }} />
              ))}
            </div>
          </Field>
          ...
          return (
            <div key={p.id} style={{ background: '#fff', border: `1px solid ${p.couleur}44`, borderRadius: 12, padding: 18, boxShadow: '0 1px 3px rgba(0,0,0,.06)', transition: 'all .15s' }}
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 12 }}>
                {[
                  { l: 'Tâches', v: pa.length, c: '#7a7672' },
                  { l: 'Validées', v: done, c: '#16a34a' },
                  { l: 'Retard', v: overdue, c: overdue > 0 ? '#dc2626' : '#a09c98' },
                ].map(({ l, v, c }) => (
              ...
              {projetAgents.length > 0 && (
                <div style={{ marginTop: 12, borderTop: '1px solid #f0ede8', paddingTop: 10 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                    {projetAgents.map(u => (
                      <span key={u.id} style={{ fontSize: 10, background: `${p.couleur}15`, color: p.couleur, padding: '2px 8px', borderRadius: 99, fontWeight: 600 }}>
                        {u.nom.split(' ')[0]}
                      </span>
                    ))}
                  </div>
